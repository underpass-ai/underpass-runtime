...
// containerK8sOps abstracts Kubernetes operations for container tools.
// Nil when K8s build tag is absent; tools fall back to local/simulated mode.
type containerK8sOps interface {
	...
	invokeRun(ctx context.Context, session domain.Session, params k8sRunParams) (app.ToolRunResult, *domain.Error)
...
// k8sRunParams holds parameters for K8s container.run invocations.
...
func (h *ContainerRunHandler) Invoke(ctx context.Context, session domain.Session, args json.RawMessage) (app.ToolRunResult, *domain.Error) {
	...
	strict := resolveContainerStrictFlag(request.Strict)
	if isKubernetesRuntime(session) && h.k8sOps != nil {
		return h.k8sOps.invokeRun(ctx, session, k8sRunParams{
			imageRef:      imageRef,
			command:       command,
			envPairs:      envPairs,

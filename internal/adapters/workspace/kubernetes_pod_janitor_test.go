...
const (
	testJanitorNamespace         = "tenant-runtime"
	testLabelAppWorkspaceSession = "workspace-session"
	testLabelAppContainerRun     = "workspace-container-run"
	testSessionExpiredID         = "session-expired"
)
...
func TestKubernetesPodJanitor_SweepDeletesTerminalContainerPods(t *testing.T) {
	...
	client := k8sfake.NewSimpleClientset(&corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace:         testJanitorNamespace,
			CreationTimestamp: metav1.NewTime(now.Add(-10 * time.Minute)),
			Labels: map[string]string{
				"app": testLabelAppContainerRun,
			},
		},
	...
	_, err := client.CoreV1().Pods(testJanitorNamespace).Get(context.Background(), "ws-ctr-session-a1b2c3", metav1.GetOptions{})
	if err == nil {
...
func TestKubernetesPodJanitor_SweepDeletesOrphanedSessionPods(t *testing.T) {
	...
	client := k8sfake.NewSimpleClientset(&corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			CreationTimestamp: metav1.NewTime(now.Add(-10 * time.Minute)),
			Labels: map[string]string{
				"app":          testLabelAppWorkspaceSession,
	...
	_, err := client.CoreV1().Pods(testJanitorNamespace).Get(context.Background(), "ws-session-orphaned", metav1.GetOptions{})
	if err == nil {
...
func TestKubernetesPodJanitor_SweepKeepsFreshPodsDuringSessionGracePeriod(t *testing.T) {
	...
	_, err := client.CoreV1().Pods(testJanitorNamespace).Get(context.Background(), "ws-session-fresh", metav1.GetOptions{})
	if err != nil {
...
func TestKubernetesPodJanitor_SweepDeletesExpiredSessionsAndStoreKey(t *testing.T) {
	...
	client := k8sfake.NewSimpleClientset(&corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"app":          testLabelAppWorkspaceSession,
				"workspace_id": testSessionExpiredID,
			},
		},
		Status: corev1.PodStatus{Phase: corev1.PodRunning},
	})
	...
	store := &fakeJanitorSessionStore{
		sessions: map[string]domain.Session{
			testSessionExpiredID: {
				ID:        testSessionExpiredID,
	...
	_, err := client.CoreV1().Pods(testJanitorNamespace).Get(context.Background(), "ws-session-expired", metav1.GetOptions{})
	if err == nil {
		t.Fatal("expected expired session pod to be deleted")
	}

	if len(store.deleted) != 1 || store.deleted[0] != testSessionExpiredID {
...
func TestKubernetesPodJanitor_SweepContainerPodsOrphanedSession(t *testing.T) {
	...
	_, err := client.CoreV1().Pods(testJanitorNamespace).Get(context.Background(), "ws-ctr-orphaned", metav1.GetOptions{})
	if err == nil {
...
func TestKubernetesPodJanitor_SweepContainerPodsExpiredSession(t *testing.T) {
	...
	client := k8sfake.NewSimpleClientset(&corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"app":                  testLabelAppContainerRun,
				"workspace_session_id": testSessionExpiredID,
			},
		},
		Status: corev1.PodStatus{Phase: corev1.PodRunning},
	})
	...
	_, err := client.CoreV1().Pods(testJanitorNamespace).Get(context.Background(), "ws-ctr-expired", metav1.GetOptions{})
	if err == nil {
